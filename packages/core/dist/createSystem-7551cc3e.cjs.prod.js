...
require('pluralize');
var nextFields = require('./next-fields-d22d3935.cjs.prod.js');
var graphqlTsSchema = require('./graphql-ts-schema-80f16062.cjs.prod.js');
var createAdminMeta = require('./create-admin-meta-bc3ac1cf.cjs.prod.js');
...
function idFieldType(config) {
  ...
  function parse(value) {
    ...
    if (result === undefined) {
      throw createAdminMeta.userInputError(`Only a ${type_.toLowerCase()} can be passed to id filters`);
    }
    return result;
  }
  return meta => {
...
function resolveDefaults(config) {
  ...
  return {
    ...
    graphql: {
      ...
      playground: (_config$graphql$playg = (_config$graphql2 = config.graphql) === null || _config$graphql2 === void 0 ? void 0 : _config$graphql2.playground) !== null && _config$graphql$playg !== void 0 ? _config$graphql$playg :         "production" !== 'production',
...
function getVariablesForGraphQLField(field) {
  const variableDefinitions = field.args.map(arg => {
    ...
    return {
      ...
      defaultValue: arg.defaultValue === undefined ? undefined : (_ref = graphql$1.astFromValue(arg.defaultValue, arg.type)) !== null && _ref !== void 0 ? _ref : undefined
    };
  });
...
function executeGraphQLFieldToRootVal(field) {
  ...
  const document = {
    ...
    definitions: [{
      ...
      operation: graphql$1.OperationTypeNode.QUERY,
      selectionSet: {
...
async function getOperationFieldAccess(item, list, fieldKey, context, operation) {
  ...
  try {
    ...
    throw createAdminMeta.extensionError('Access control', [{
      error,
      tag: `${list.listKey}.${fieldKey}.access.${operation}`
    }]);
  }
  if (typeof result !== 'boolean') {
    throw createAdminMeta.accessReturnError([{
...
async function getAccessFilters(list, context, operation) {
  try {
    ...
    const whereInput = graphql$1.assertInputObjectType(schema.getType(list.graphql.names.whereInputName));
...
async function enforceListLevelAccessControl(context, operation, list, inputData, item) {
  ...
  throw createAdminMeta.accessDeniedError(cannotForItem(operation, list));
}
async function enforceFieldLevelAccessControl(context, operation, list, inputData, item) {
  ...
  if (nonBooleans.length) {
    throw createAdminMeta.accessReturnError(nonBooleans);
  }
  if (accessErrors.length) {
    throw createAdminMeta.extensionError('Access control', accessErrors);
  }
  if (fieldsDenied.length) {
    throw createAdminMeta.accessDeniedError(cannotForItemFields(operation, list, fieldsDenied));
  }
}
    ...
    AND: await Promise.all(Object.entries(inputFilter).map(async ([key, value]) => {
      ...
      const ret = resolve ? await resolve(value, context, (() => {
        ...
        if (dbField.mode === 'many') {
          return async () => {
            if (value === null) {
              throw createAdminMeta.userInputError('A many relation filter cannot be set to null');
            }
            return Object.fromEntries(await Promise.all(Object.entries(value).map(async ([key, val]) => {
              if (val === null) {
                throw createAdminMeta.userInputError(`The key "${key}" in a many relation filter cannot be set to null`);
              }
...
async function checkFilterOrderAccess(things, context, operation) {
  ...
  if (accessErrors.length) {
    throw createAdminMeta.extensionError(func, accessErrors);
  }
  if (returnTypeErrors.length) {
    throw createAdminMeta.accessReturnError(returnTypeErrors);
  }
  if (failures.length) {
    throw createAdminMeta.filterAccessError({
      operation,
      fieldKeys: failures
    });
  }
...
async function findMany({
  ...
  if ((take !== null && take !== void 0 ? take : Infinity) > maxTake) {
    throw createAdminMeta.limitsExceededError({
      list: list.listKey,
      type: 'maxTake',
      limit: maxTake
    });
...
async function resolveOrderBy(orderBy, list, context) {
  ...
  orderBy.forEach(orderBySelection => {
    ...
    if (keys.length !== 1) {
      throw createAdminMeta.userInputError(`Only a single key must be passed to ${list.graphql.types.orderBy.graphQLType.name}`);
    }
    const fieldKey = keys[0];
    const value = orderBySelection[fieldKey];
    if (value === null) {
      throw createAdminMeta.userInputError('null cannot be passed as an order direction');
    }
  });
...
function getQueriesForList(list) {
  ...
  const findOne$1 = graphqlTsSchema.field({
  ...
  const findMany$1 = graphqlTsSchema.field({
...
async function validate({
  ...
  if (fieldsErrors.length) {
    throw createAdminMeta.extensionError('validateInput', fieldsErrors);
  }
  ...
  // list validation hooks
  {
    ...
    try {
      ...
      throw createAdminMeta.extensionError('validateInput', [{
        error,
        tag: `${list.listKey}.hooks.validateInput`
      }]);
    }
    if (messages.length) {
      throw createAdminMeta.validationFailureError(messages);
    }
  }
}
async function runSideEffectOnlyHook(list, hookName, args) {
  ...
  if (fieldsErrors.length) {
    throw createAdminMeta.extensionError(hookName, fieldsErrors);
  }
  ...
  try {
    ...
    throw createAdminMeta.extensionError(hookName, [{
      error,
...
async function checkUniqueItemExists(uniqueInput, foreignList, context, operation) {
  ...
  throw createAdminMeta.accessDeniedError(cannotForItem(operation, foreignList));
}
...
function resolveRelateToManyForCreateInput(nestedMutationState, context, foreignList, tag) {
  return async value => {
    if (!Array.isArray(value.connect) && !Array.isArray(value.create)) {
      throw createAdminMeta.userInputError(`You must provide "connect" or "create" in to-many relationship inputs for "create" operations.`);
    }
...
function resolveRelateToManyForUpdateInput(nestedMutationState, context, foreignList, tag) {
  return async value => {
    if (!Array.isArray(value.connect) && !Array.isArray(value.create) && !Array.isArray(value.disconnect) && !Array.isArray(value.set)) {
      throw createAdminMeta.userInputError(`You must provide at least one of "set", "connect", "create" or "disconnect" in to-many relationship inputs for "update" operations.`);
    }
    if (value.set && value.disconnect) {
      throw createAdminMeta.userInputError(`The "set" and "disconnect" fields cannot both be provided to to-many relationship inputs for "update" operations.`);
    }
...
function resolveRelateToOneForCreateInput(nestedMutationState, context, foreignList) {
  return async value => {
    ...
    if (numOfKeys !== 1) {
      throw createAdminMeta.userInputError(`You must provide "connect" or "create" in to-one relationship inputs for "create" operations.`);
    }
...
function resolveRelateToOneForUpdateInput(nestedMutationState, context, foreignList) {
  return async value => {
    if (Object.keys(value).length !== 1) {
      throw createAdminMeta.userInputError(`You must provide one of "connect", "create" or "disconnect" in to-one relationship inputs for "update" operations.`);
    }
...
class NestedMutationState {
  ...
  async create(data, list) {
    ...
    if (!operationAccess) throw createAdminMeta.accessDeniedError(cannotForItem('create', list));
    const {
      item,
      afterOperation
...
async function updateOne(updateInput, list, context) {
  ...
  if (!operationAccess) throw createAdminMeta.accessDeniedError(cannotForItem('update', list));
...
async function deleteOne(where, list, context) {
  ...
  if (!operationAccess) throw createAdminMeta.accessDeniedError(cannotForItem('delete', list));
...
async function getResolvedData(list, hookArgs, nestedMutationState) {
  ...
  if (resolverErrors.length) throw createAdminMeta.resolverError(resolverErrors);
  ...
  if (relationshipErrors.length) throw createAdminMeta.relationshipError(relationshipErrors);
  ...
  if (fieldsErrors.length) throw createAdminMeta.extensionError('resolveInput', fieldsErrors);
  ...
  try {
    ...
    throw createAdminMeta.extensionError('resolveInput', [{
      error,
...
function nonNull(t) {
  if (t === graphqlTsSchema.Empty) return t;
  return apiWithoutContext.nonNull(t);
}
function getMutationsForList(list) {
  const defaultUniqueWhereInput = list.isSingleton ? {
    id: '1'
  } : undefined;
  const createOne_ = graphqlTsSchema.field({
    type: list.graphql.types.output,
    args: {
  ...
  const createMany_ = graphqlTsSchema.field({
    type: apiWithoutContext.list(list.graphql.types.output),
    args: {
      data: apiWithoutContext.arg({
        type: apiWithoutContext.nonNull(apiWithoutContext.list(nonNull(list.graphql.types.create)))
      })
    },
  ...
  const updateOne_ = graphqlTsSchema.field({
    type: list.graphql.types.output,
    args: {
      where: apiWithoutContext.arg({
        type: apiWithoutContext.nonNull(list.graphql.types.uniqueWhere),
  ...
  const updateMany_ = graphqlTsSchema.field({
    type: apiWithoutContext.list(list.graphql.types.output),
    args: {
      data: apiWithoutContext.arg({
        type: apiWithoutContext.nonNull(apiWithoutContext.list(nonNull(updateManyInput)))
      })
    },
    async resolve(_rootVal, {
      data
  ...
  const deleteOne_ = graphqlTsSchema.field({
    type: list.graphql.types.output,
    args: {
      where: apiWithoutContext.arg({
        type: apiWithoutContext.nonNull(list.graphql.types.uniqueWhere),
  ...
  const deleteMany_ = graphqlTsSchema.field({
    type: apiWithoutContext.list(list.graphql.types.output),
    args: {
      where: apiWithoutContext.arg({
...
function localFileAssetsAPI(storageConfig) {
  return {
    ...
    async upload(stream, filename) {
      const writeStream = fs__default$1["default"].createWriteStream(path__default["default"].join(storageConfig.storagePath, filename));
      const pipeStreams = new Promise((resolve, reject) => {
        node_stream.pipeline(stream, writeStream, err => {
          if (err) {
            reject(err);
          } else {
            resolve();
      ...
      try {
        ...
        await fs__default["default"].rm(path__default["default"].join(storageConfig.storagePath, filename));
        throw e;
      }
    },
...
function createContext({
  ...
  const construct = ({
    ...
    const rawGraphQL = async ({
      ...
      return await graphql$1.graphql({
        schema,
        source,
        contextValue: context,
...
function getListsWithInitialisedFields(config, listsRef) {
  ...
  for (const listConfig of Object.values(listsConfig)) {
    ...
    } = nextFields.__getNames(listKey, listConfig);
    ...
    const relateToOneForCreate = apiWithoutContext.inputObject({
    ...
    const relateToOneForUpdate = apiWithoutContext.inputObject({
    ...
    const relateToManyForCreate = apiWithoutContext.inputObject({
      ...
      fields: () => {
        ...
        return {
          ...(listRef.graphql.isEnabled.create && {
            create: apiWithoutContext.arg({
              type: apiWithoutContext.list(apiWithoutContext.nonNull(listRef.graphql.types.create))
            })
          }),
          connect: apiWithoutContext.arg({
            type: apiWithoutContext.list(apiWithoutContext.nonNull(listRef.graphql.types.uniqueWhere))
          })
        };
      }
    });
    const relateToManyForUpdate = apiWithoutContext.inputObject({
  ...
  for (const listConfig of Object.values(listsConfig)) {
    ...
    const names = nextFields.__getNames(listKey, listConfig);
    result[listKey] = {
...
function introspectGraphQLTypes(lists) {
  for (const list of Object.values(lists)) {
    ...
    for (const fieldKey of Object.keys(list.fields)) {
      ...
      if ((fieldFilterFields === null || fieldFilterFields === void 0 || (_fieldFilterFields$co = fieldFilterFields.contains) === null || _fieldFilterFields$co === void 0 ? void 0 : _fieldFilterFields$co.type) === graphql$1.GraphQLString) {
        var _fieldFilterFields$mo;
        searchableFields.set(fieldKey, (fieldFilterFields === null || fieldFilterFields === void 0 || (_fieldFilterFields$mo = fieldFilterFields.mode) === null || _fieldFilterFields$mo === void 0 ? void 0 : _fieldFilterFields$mo.type) === nextFields.QueryMode.graphQLType ? 'insensitive' : 'default');
      }
    }
...
function initialiseLists(config) {
  ...
  for (const list of Object.values(intermediateLists)) {
    ...
    if (!hasAnEnabledCreateField) {
      list.graphql.types.create = graphqlTsSchema.Empty;
      list.graphql.names.createInputName = 'Empty';
    }
    if (!hasAnEnabledUpdateField) {
      list.graphql.types.update = graphqlTsSchema.Empty;
...
function getSystemPaths(cwd, config) {
  ...
  : path__default["default"].join(cwd, 'node_modules/.keystone/types.ts');
    ...
    query: {
      async $allOperations({
        ...
        try {
          ...
          console.error(e);
          if (e.code === undefined) {
            return new graphql$1.GraphQLError(`Prisma error`, {
              extensions: {
          ...
          return new graphql$1.GraphQLError(`Prisma error: ${(_e$message$split$pop = e.message.split('\n').pop()) === null || _e$message$split$pop === void 0 ? void 0 : _e$message$split$pop.trim()}`, {
            extensions: {
              code: 'KS_PRISMA_ERROR',
              prisma: {
                ...e
