...
import { C as CreateItemDrawer } from '../../../../../dist/CreateItemDrawer-e7977ef7.esm.js';
...
import { d as deserializeValue, u as useChangedFieldsAndDataForUpdate, a as useInvalidFields, s as serializeValueToObjByFieldKey } from '../../../../../dist/useInvalidFields-51250ca4.esm.js';
import 'next/head';
...
function useFieldsObj(list, fields) {
  return useMemo(() => {
    const editFields = {};
    fields === null || fields === void 0 || fields.forEach(fieldPath => {
...
function InlineCreate({
  ...
  return jsx("section", null, jsx(Stack, {
    gap: "xlarge"
  }, error && jsx(GraphQLErrorNotice, {
    networkError: error === null || error === void 0 ? void 0 : error.networkError,
...
function Cards({
  ...
  useEffect(() => {
    if (value.itemsBeingEdited) {
      ...
      editRef === null || editRef === void 0 || (_editRef$current = editRef.current) === null || _editRef$current === void 0 || _editRef$current.focus();
    }
  }, [value]);
  ...
  }, currentIdsArrayWithFetchedItems.map(({
    id,
    itemGetter
  }, index) => {
        ...
        if (itemsToFetchAndConnect.length) {
          try {
            ...
            if (isMountedRef.current) {
              ...
              const newCurrentIds = field.many ? new Set(value.currentIds) : new Set();
  ...
  return {
    ...
    deserialize: data => {
      ...
      if (config.fieldMeta.many) {
        const value = (data[config.path] || []).map(x => ({
          id: x.id,
          label: x.label || x.id
        }));
        return {
    ...
    serialize: state => {
      if (state.kind === 'many') {
        ...
        const connect = state.value.filter(x => !initialIds.has(x.id)).map(x => ({
          id: x.id
        }));
        ...
        const disconnect = [...state.initialIds].filter(id => !state.currentIds.has(id)).map(id => ({
          id
        }));
        const connect = [...state.currentIds].filter(id => !state.initialIds.has(id)).map(id => ({
          id
        }));
        if (config.fieldMeta.many) {
          if (disconnect.length || connect.length) {
...
function useRelationshipFilterValues({
  ...
  return {
    filterValues: (data === null || data === void 0 || (_data$items = data.items) === null || _data$items === void 0 ? void 0 : _data$items.map(item => {
      return {
        id: item.id,
